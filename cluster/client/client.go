...
func (client *Client) Send(args [][]byte) respinterface.Reply {
	request := &request{
		args:      args,
		heartbeat: false,
		waiting:   &wait.Wait{},
...
func (client *Client) finishRequest(reply respinterface.Reply) {
	defer func() {
...
func (client *Client) handleRead() error {
	...
	for stream := range ch {
		if stream.Err != nil {
			client.finishRequest(reply.NewErrReply(stream.Err.Error()))
			continue
		}
		client.finishRequest(stream.Data)
	}
	return nil
}
